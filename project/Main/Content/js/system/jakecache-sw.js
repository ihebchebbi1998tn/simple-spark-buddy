...
class JakeCacheManifest {
  ...
  groupName () {
    let filename = this._path.startsWith("/") ? this._path.substring(1) : this._path;
    return filename
  }
  async read(response) {
    let decoded = '';
    ...
    while (!done) {
      ...
      decoded += decoder.decode(result.value || new Uint8Array(), { stream: !done });
    }
    return decoded;
  };
  async fetchData (path, options = {}) {
    ...
    let cachedHash = null;
    var response = await caches.match(path);
    if (response) {
      this._rawData = {
        cache: [],
        fallback: [],
      ...
      const decoded = await this.read(response);
      cachedHash = md5(decoded)
    }

    // http://html5doctor.com/go-offline-with-application-cache/
    return fetch(new Request(this._path, options)).then((response) => {
      if (response.type === 'opaque' || response.status === 401 || response.status === 403 || response.status === 404 || response.status === 410) {
      ...
      return new Promise(async (resolve, reject) => {
        const decoded = await this.read(response);
          ...
          if (cachedHash && hash.toString() === cachedHash.toString()) {
            console.log('noupdate: ' + hash)
            return resolve(false)
          }
          console.log(`update: ${hash} (was: ${cachedHash})`)
...
function postMessage (msg) {
  return self.clients.matchAll().then(clients => {
    if (clients.length === 0) {
      return self.clients.claim().then(() => self.clients.matchAll());
    }
...
function update (pathname, options = {}) {
  ...
  return caches.keys().then(cacheNames => {
    ...
    this.requests.push(new Request(this.cacheGroup, { mode: 'no-cors' }));
    ...
    if (responses.length != self.requests.length) {
      throw Error();
    }
    return caches.open(manifest.groupName()).then(cache => {
      let manifestRequest = self.requests.find(x => x.url.endsWith("/" + manifest.groupName()));
      ...
      return promise.then(() => {
        ...
        let keysToDelete = keys.filter(key => self.requests.find(x => x.url === key.url) === undefined);
        return Promise.all(keysToDelete.map(key => {
          return cache.delete(key);
        }));
      }).then(() => {
        return cache.put(manifestRequest, responses[self.requests.indexOf(manifestRequest)]);
      });
    }).then(_ => {
      if (this.upgrade) {
