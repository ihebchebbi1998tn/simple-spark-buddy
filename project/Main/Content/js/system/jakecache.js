...
if (navigator.userAgent.indexOf("OfflineClient") !== -1 && 'applicationCache' in window){
	return;
}
  ...
  constructor () {
    ...
    let onload = () => {
      ...
      if (this.pathname && 'serviceWorker' in navigator) {
        navigator.serviceWorker.register(window.Helper.Url.resolveUrl('~/Plugins/Main/Content/js/system/jakecache-sw.js'), { scope: window.Helper.Url.resolveUrl('~/') }).then(registration => {
          console.log(`JakeCache installed for ${registration.scope}`)
            ...
            if (registration.active) {
              ...
              navigator.serviceWorker.ready.then(function() {
                registration.active.postMessage({
                  command: 'update',
                  pathname: window.jakeCache.pathname,
                  options: {
                    cache: 'reload'
                  }
                })
