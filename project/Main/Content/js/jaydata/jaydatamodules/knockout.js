...
const dateEdmTypes = ['Edm.DateTime', 'Edm.DateTimeOffset'];
const dateObservable = function (initialValue) {
	const observable = _ko2.default.observable(initialValue);
	const originalComparer = observable.equalityComparer;
	observable.equalityComparer = (a, b) => (a instanceof Date && b instanceof Date) ? a.getTime() === b.getTime() : originalComparer(a, b);
    ...
    function ObservableFactory(originalType, observableClassNem) {
        ...
        for (var i = 0, l = properties.length; i < l; i++) {
            ...
            instanceDefinition[propName] = {
                type: properties[i].type == Array ? _ko2.default.observableArray : dateEdmTypes.includes(properties[i].originalType) ? dateObservable : _ko2.default.observable
            };
