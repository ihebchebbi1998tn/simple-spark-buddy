(function (ko) {
	...
	ko.bindingHandlers.translatedDataWmlSpeechCommand = {
		update: function (element: HTMLElement, valueAccessor) {
			...
			if (translationMissing) {
				window.Helper.String.fetchStringTranslation(valueUnwrapped, language).then(function (fetchedTranslation) {
					element.setAttribute("data-wml-speech-command", fetchedTranslation);
				});
			} else {
				element.setAttribute("data-wml-speech-command", translation);
			}
		}
	};
})(window.ko);
